<x-default-layout :title="$encyclopedia->title">
        {{-- Corps --}}
                        ...
                        <div class="w-full lg:m-3 mt-3">
                            ...
                            {{-- Contenu de l'article--}}
                                <div class="flex flex-col">
                                    <a class="lg:text-4xl text-xl font-bold text-[#3e2610] text-justify">{{ $encyclopedia->title }}</a>
                                    <p class="mb-3 text-justify">{!! nl2br(e($encyclopedia->content)) !!}</p>
        
                                    @if ($post->category)
                                    <a href="{{ route('encyclopedias.byType', ['type'=>$encyclopedia->type]) }}" class="font-bold">{{ $encyclopedia->type->name }}</a>
                                    @endif

                                    @if($encyclopedia->)
                                    <a href="{{ route('encyclopedias.bySubType', ['type'=>$encyclopedia->type]) }}" class="font-bold">{{ $encyclopedia->type->name }}</a>
                                    @endif
                            ...
                            <div class="m-2 lg:w-1/3 flex flex-row">
                                ...
                                <img alt="Image" class="justify-end" src="{{$encyclopedia->thumbnail}}">
