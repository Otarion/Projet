...
<div class="w-full lg:m-3 mt-3">
    ...
    <div class="lg:flex flex-row lg:w-5/6 w-11/12 mx-auto lg:h-72">
        <div class="flex flex-row m-2 lg:w-2/3">

            <div class="flex flex-col">
            <a class="lg:text-4xl text-xl font-bold text-[#3e2610] text-justify" href="">{{ $post->title }}</a>
            <p class="mb-3 text-justify">{!! nl2br(e($post->content)) !!}</p>
            <a>Catégorie</a>
            <time class="text-[#a5a5a5]" datetime="{{ $post->created_at }}">{{ $post->created_at }}</time>
            </div>
